...
A genuine, if narrow, finding: verify_range16 in the Madiba gadgets crate checks that some value under 2^16 was proven, but never binds it to the specific scalar the caller cares about. That's a real soundness gap in a third-party dependency, not a bug in your the code. verify_range16_bound fixes it, but only as a local wrapper in the codebase.
