...
**Note (this copy only, `zk_fba_real_data/rust/`):** this file predates
two additive-only chunks that this project (not `zk_fba_csv_full`) later
added on top of the code guide below, and it does not describe them:
(1) an independent, external `verify_external` verifier + serializable
`ExternalProof` (new code in `src/lib.rs`), and (2) a parallel
RLC-aggregated bit-gadget pipeline (`src/agg_bit_gadget.rs` +
`src/agg_pipeline.rs`) that cuts the 390-polynomial bit-decomposition
layer described below down to 204 polynomials for a smaller proof. Both
are strictly additive -- nothing described in this file was changed or
removed -- see `../RESULTS.md`'s "External auditability (Chunk 1)" and
"RLC-aggregated bit-gadget pipeline for proof size (Chunk 4)" sections
for what they do and why, and `../SYNTHETIC_VS_REAL_COMPARISON.md` for
how they affect the Rust-vs-Noir comparison.
