...
//!
//! Rationale, design decisions, and benchmarks: see RATIONALE_AND_RESULTS.md.
...
use gadgets::utils::{
    batch_check, batch_open, convert_to_bigints, skip_leading_zeros_and_convert_to_bigints,
    BatchCheckProof, OpenEval, Transcript,
};
use rayon::prelude::*;

// New, additive-only modules (RLC-aggregated bit-gadget redesign, ported
// verbatim from ~/zk_fba_csv_full/src/{agg_bit_gadget,agg_pipeline}.rs).
// Nothing above/below this line in lib.rs is modified for them -- see
// agg_bit_gadget.rs's and agg_pipeline.rs's module doc comments.
pub mod agg_bit_gadget;
...
/// Caches the coset domain + inverted `Z_H` evaluations built inside
/// `divide_by_vanishing_coset`, keyed by `m` (the coset size, derived from
/// the numerator's degree). Many numerators passed to that function within
/// one `compute_quotients`/`build_bit_gadget_set` call share the same degree
/// class (e.g. every bit-booleanity check `b*(b-1)` has the same degree for
/// a fixed `domain_size`), so `m` repeats often; caching by `m` reuses the
/// coset + `Z_H` setup instead of rebuilding it from scratch on every call.
/// `domain_size` is constant for the lifetime of one cache (one pipeline
/// run), so it is not part of the key.
#[derive(Default)]
pub struct CosetCache {
    map: HashMap<usize, (GeneralEvaluationDomain<F>, Vec<F>)>,
}
...
impl CosetCache {
    ...
    fn get_or_build(&mut self, m: usize, domain_size: usize) -> &(GeneralEvaluationDomain<F>, Vec<F>) {
        self.map.entry(m).or_insert_with(|| {
            let coset = GeneralEvaluationDomain::<F>::new(m)
                .expect("coset domain creation failed")
                .get_coset(F::GENERATOR)
                .expect("coset construction failed");
    ...
    /// Populate the cache entry for `m` if absent. Mutable/sequential by
    /// design: call this for every distinct `m` a batch of numerators will
    /// need *before* reading them back with `get` from parallel closures,
    /// so the parallel section only ever takes shared (`&self`) borrows.
    fn ensure(&mut self, m: usize, domain_size: usize) {
        self.get_or_build(m, domain_size);
    }

    /// Read-only lookup for use inside a `par_iter` closure. Panics if
    /// `ensure` was not called first for this `m` -- that would be a bug in
    /// the caller, not a legitimate runtime state.
    fn get(&self, m: usize) -> &(GeneralEvaluationDomain<F>, Vec<F>) {
        self.map.get(&m).expect("CosetCache::get called before ensure")
    }
}

/// Same computation as `divide_by_vanishing_coset`, but reuses a shared
/// `CosetCache` for the coset domain + `Z_H` evaluations instead of
/// rebuilding them on every call. Numerically identical to
/// `divide_by_vanishing_coset` for the same inputs -- this is a caching
/// refactor, not an algorithm change.
pub fn divide_by_vanishing_coset_cached(
    numerator:   &DensePolynomial<F>,
    domain_size: usize,
    ...
    let num_deg = numerator.degree();
    let m = (num_deg + 1).next_power_of_two();
    let (coset, zh_evals) = cache.get_or_build(m, domain_size);
...
/// Same computation again, but takes a shared `&CosetCache` instead of
/// `&mut`, for use inside a `par_iter` closure where multiple independent
/// numerators are processed concurrently. The caller must have already
/// called `CosetCache::ensure` for every `m` this batch of numerators will
/// need (see `build_bit_gadget`'s booleanity loop for the pattern:
/// sequential `ensure` pass first, then a read-only parallel pass).
fn divide_by_vanishing_coset_ro(
    numerator: &DensePolynomial<F>,
    cache:     &CosetCache,
) -> DensePolynomial<F> {
    if numerator.coeffs.is_empty() {
    ...
    let num_deg = numerator.degree();
    let m = (num_deg + 1).next_power_of_two();
    let (coset, zh_evals) = cache.get(m);
...
pub fn compute_quotients(
    ...
    let mut coset_cache = CosetCache::new();
    let omega_0         = elems[0];
    ...
    let q2 = divide_by_vanishing_coset_cached(&v2, domain_size, &mut coset_cache);
    ...
    let q4 = divide_by_vanishing_coset_cached(&v4, domain_size, &mut coset_cache);
    ...
    let v5 = &da * &db;
    let q5 = divide_by_vanishing_coset_cached(&v5, domain_size, &mut coset_cache);
    ...
    let q6 = divide_by_vanishing_coset_cached(&v6, domain_size, &mut coset_cache);
    ...
    let q7 = divide_by_vanishing_coset_cached(&v7, domain_size, &mut coset_cache);
    ...
    let q8 = divide_by_vanishing_coset_cached(&v8, domain_size, &mut coset_cache);
    let r8_ok = quotient_holds_at(&v8, &q8, domain_size, F::GENERATOR);

    // #20  ChkD * (1 - Mask_P) = 0
    let v9 = &polys.chk_d * &(&one_poly - mask_p);
    let q9 = divide_by_vanishing_coset_cached(&v9, domain_size, &mut coset_cache);
    ...
    let q13 = divide_by_vanishing_coset_cached(&v13, domain_size, &mut coset_cache);
    ...
    let q14 = divide_by_vanishing_coset_cached(&v14, domain_size, &mut coset_cache);
...
pub fn build_bit_gadget(
    ...
    // The BIT_WIDTH booleanity checks (`b*(b-1)` per bit column) are
    // completely independent of each other -- compute every numerator
    // first (cheap), warm the cache sequentially for whatever distinct
    // coset sizes they need (mutable, so it must happen before the
    // parallel section), then divide by the vanishing polynomial for all
    // of them concurrently via rayon, since each one's FFT/interpolate is
    // otherwise the dominant per-bit cost.
    let one_poly = DensePolynomial { coeffs: vec![F::one()] };
    let bool_numerators: Vec<DensePolynomial<F>> =
        cols.bits.iter().map(|b| b * &(b - &one_poly)).collect();
    for v in &bool_numerators {
        if !v.coeffs.is_empty() {
            let m = (v.degree() + 1).next_power_of_two();
            cache.ensure(m, domain_size);
        }
    }
    let cache_ro: &CosetCache = cache;
    let q_bools: Vec<DensePolynomial<F>> = bool_numerators
        .par_iter()
        .map(|v| divide_by_vanishing_coset_ro(v, cache_ro))
        .collect();
    let bool_ok = bool_numerators
        .iter()
        .zip(q_bools.iter())
        .all(|(v, q)| quotient_holds_at(v, q, domain_size, F::GENERATOR));
...
/// Drop-in parallel replacement for calling `scheme.commit` once per
/// polynomial in a loop. `KZG10::commit` (see ark-poly-commit's
/// `kzg10::mod.rs`) does two independent things per polynomial: draw a
/// degree-1 hiding `Randomness` from the RNG, then run two MSMs (one over
/// the polynomial's own coefficients, one over the 2-coefficient blinding
/// polynomial) that don't touch the RNG at all. Since an `R: Rng` can't
/// safely be shared across threads, all randomness is drawn up front in
/// the original sequential order (cheap: 2 field elements per polynomial),
/// then the MSM work -- the actual cost for domain-sized polynomials --
/// runs across threads. Same per-polynomial math as `scheme.commit`,
/// same `hiding_bound = 1`, just batched instead of called in a loop.
pub fn parallel_commit_many<R: Rng>(
    scheme: &KzgScheme,
    polys:  &[&DensePolynomial<F>],
    rng:    &mut R,
) -> Vec<(Commitment<Bn254>, KzgRand)> {
    let randoms: Vec<KzgRand> = (0..polys.len())
        .map(|_| KzgRand::rand(1, false, None, rng))
        .collect();

    polys
        .par_iter()
        .zip(randoms.into_par_iter())
        .map(|(&poly, randomness)| {
            let (num_leading_zeros, plain_coeffs) = skip_leading_zeros_and_convert_to_bigints(poly);
            let mut commitment = G1Projective::msm_bigint(
                &scheme.powers.powers_of_g[num_leading_zeros..],
                &plain_coeffs,
            );
            let random_ints = convert_to_bigints(randomness.blinding_polynomial.coeffs());
            let random_commitment =
                G1Projective::msm_bigint(&scheme.powers.powers_of_gamma_g, &random_ints);
            commitment += &random_commitment;
            (Commitment(commitment.into_affine()), randomness)
        })
        .collect()
}
...
pub fn commit_bit_gadget<R: Rng>(
    ...
    let bit_polys: Vec<&DensePolynomial<F>> = gadget.cols.bits.iter().collect();
    let (c_bits, r_bits): (Vec<_>, Vec<_>) =
        parallel_commit_many(scheme, &bit_polys, rng).into_iter().unzip();

    let (c_recon, r_recon) = scheme.commit(&gadget.q_recon, rng);

    let bool_polys: Vec<&DensePolynomial<F>> = gadget.q_bools.iter().collect();
    let (c_bools, r_bools): (Vec<_>, Vec<_>) =
        parallel_commit_many(scheme, &bool_polys, rng).into_iter().unzip();
...
/// BitGadgetCommitments. Consumed by `bit_gadget_set_polys_and_rands` /
/// `compute_all_opening_proofs` to actually batch-open these polynomials
/// (Phase 3).
...
/// and never influenced the verifier's challenge. All three phases are done
/// now: Phase 1 binds these commitments to the challenge (this struct,
/// absorbed in `fiat_shamir_prove`), Phase 2 folds their residues into
/// `r`/`batch_ok` (`bit_gadget_residues_and_evals`), and Phase 3 opens them
/// against a real pairing via `batch_check` (`compute_all_opening_proofs`'s
/// `bg_polys` group, using `all_commitments` below).
...
impl AllBitGadgetCommitments {
    ...
    /// Same 390-commitment set as `all_affines`, same fixed per-gadget order
    /// (bits, recon, bools; bid_range/ask_range/ceiling/surp_b_nn/surp_a_nn/
    /// delta_floor), but as `Commitment<Bn254>` rather than raw `G1Affine`.
    /// Used by `compute_all_opening_proofs` (Phase 3) to build the
    /// `BatchCheckProof` commitment group that pairs with the batch-opened
    /// bit-gadget polynomials -- this must stay in the exact same order as
    /// `bit_gadget_set_polys_and_rands` builds the polynomial/randomness
    /// vectors, since `batch_check` matches `commitments[i]` to
    /// `open_evals[i]` purely by index.
    pub fn all_commitments(&self) -> Vec<Commitment<Bn254>> {
        ...
        for bc in [
            ...
            out.extend(bc.c_bits.iter().cloned());
            out.push(bc.c_recon.clone());
            out.extend(bc.c_bools.iter().cloned());
        }
        out
    }
}
...
pub fn build_bit_gadget_set(
    ...
    // One cache shared across all six instances below: every booleanity
    // check (`b*(b-1)`) has the same numerator degree class for a fixed
    // `domain_size`, so the coset + Z_H setup built for the first instance's
    // first bit column is reused for the other ~197 `divide_by_vanishing_coset`
    // calls instead of being rebuilt from scratch each time.
    ...
    let bid_range = build_bit_gadget(&bid_bits, &polys.b, None, domain, &mut cache);

    let ask_bits: Vec<u64> = book.a.iter().map(|&f| to_u64(f)).collect();
    assert_fits_bit_width(&ask_bits, "#2 Ask range");
    let ask_range = build_bit_gadget(&ask_bits, &polys.a, None, domain, &mut cache);
    ...
    let ceiling = build_bit_gadget(&ceiling_bits, &ceiling_value, None, domain, &mut cache);
    ...
    let surp_b_nn = build_bit_gadget(&surp_b_bits, &polys.surp_b, None, domain, &mut cache);
    ...
    let surp_a_nn = build_bit_gadget(&surp_a_bits, &polys.surp_a, None, domain, &mut cache);
    ...
    let delta_floor =
        build_bit_gadget(&delta_floor_bits, &delta_floor_value, Some(mask_p), domain, &mut cache);
...
/// Flatten a `BitGadgetSet` plus its matching `AllBitGadgetRands` into the
/// polynomial/randomness vectors `batch_open` wants for Phase 3 (opening the
/// six bit-gadget instances for real, instead of only folding their
/// residues into the Fiat-Shamir sum-check as Phase 2 did). Order matches
/// `AllBitGadgetCommitments::all_commitments` exactly: per gadget, bits then
/// recon then bools; gadgets in bid_range/ask_range/ceiling/surp_b_nn/
/// surp_a_nn/delta_floor order. That matching order is what lets
/// `batch_check` pair `commitments[i]` with `open_evals[i]` correctly.
fn bit_gadget_set_polys_and_rands<'a>(
    gadgets: &'a BitGadgetSet,
    rands:   &'a AllBitGadgetRands,
    ...
    for (g, r) in [
        ...
        polys.extend(g.cols.bits.iter());
        rnds.extend(r.r_bits.iter());
        polys.push(&g.q_recon);
        rnds.push(&r.r_recon);
        polys.extend(g.q_bools.iter());
        rnds.extend(r.r_bools.iter());
    }
    (polys, rnds)
}
...
/// Drop-in replacement for `gadgets::utils::batch_open`, used only for
/// Group 2 (the ~390-polynomial bit-gadget opening, by far the largest
/// group -- see `compute_all_opening_proofs`). `batch_open` itself loops
/// over its polynomials with a plain sequential `for`, not gated by
/// arkworks' `parallel` feature (that feature only threads *within* a
/// single FFT/MSM call, and does nothing for a hand-written outer loop
/// like this one), so with ~390 polynomials it becomes the single most
/// expensive step in the whole prover. Every per-polynomial computation
/// below (witness-polynomial division, the two evaluations, the two curve
/// scalar multiplications for `committed_eval`) depends only on that one
/// polynomial, its own randomness, and `gamma.pow(i)` -- none of it depends
/// on any other iteration -- so it is exactly the same math as
/// `batch_open`, just computed across threads instead of one at a time,
/// with the two running sums (`w`, `rand_h`) folded afterward. Produces the
/// identical `(G1, Vec<OpenEval>, gamma)` triple `batch_open` would for the
/// same inputs, and is verified by the same *unmodified* `batch_check` --
/// any mismatch here would surface as a pairing-check failure, not pass
/// silently.
pub fn parallel_batch_open<R: Rng>(
    powers:       &Powers<Bn254>,
    polys:        &[&DensePolynomial<F>],
    randoms:      &[&KzgRand],
    point:        F,
    hiding_evals: bool,
    rng:          &mut R,
) -> (G1Projective, Vec<OpenEval<Bn254>>, F) {
    assert_eq!(polys.len(), randoms.len());
    let gamma = F::rand(rng);

    let per_poly: Vec<(DensePolynomial<F>, DensePolynomial<F>, F, F, G1Affine)> = polys
        .par_iter()
        ...
        .map(|(i, (&p, &random))| {
            ...
            let (witness, random_witness) =
                KzgBn254::compute_witness_polynomial(p, point, random).unwrap();
            let factor = gamma.pow(&[i as u64]);
            let weighted_witness      = witness.mul(factor);
            let weighted_rand_witness = random_witness.unwrap().mul(factor);
            let committed_eval =
                powers.powers_of_g[0].mul(eval) + powers.powers_of_gamma_g[0].mul(blinding_eval);
            (weighted_witness, weighted_rand_witness, eval, blinding_eval, committed_eval.into_affine())
        })
        .collect();

    let mut w        = DensePolynomial::<F>::zero();
    let mut rand_h    = DensePolynomial::<F>::zero();
    let mut plain_evals     = Vec::with_capacity(per_poly.len());
    let mut committed_evals = Vec::with_capacity(per_poly.len());
    for (ww, wr, eval, blind, ce) in per_poly {
        w = w + ww;
        rand_h = rand_h + wr;
        plain_evals.push((eval, blind));
        committed_evals.push(ce);
    }

    let (num_leading_zeros, witness_coeffs) = skip_leading_zeros_and_convert_to_bigints(&w);
    let mut h = G1Projective::msm_bigint(&powers.powers_of_g[num_leading_zeros..], witness_coeffs.as_slice());
    let random_witness_coeffs = convert_to_bigints(&rand_h.coeffs());
    h += &G1Projective::msm_bigint(&powers.powers_of_gamma_g, random_witness_coeffs.as_slice());

    let open_evals: Vec<OpenEval<Bn254>> = match hiding_evals {
        true  => committed_evals.into_iter().map(OpenEval::Committed).collect(),
...
pub fn compute_all_opening_proofs<R: Rng>(
    ...
    // Group 2: all 390 bit-gadget polynomials (32 bits + 1 recon quotient +
    // 32 bool quotients, x 6 instances) at zeta. This is Phase 3: it's what
    // actually pairing-verifies that the evaluations `fiat_shamir_prove`
    // folded into the #1/#2/#8/#15/#16/#23 residues really are the openings
    // of the committed polynomials in `bgcomms`, not just self-consistent
    // values the prover picked without those commitments constraining them.
    // Before this, `bgcomms` was only absorbed into the transcript (Phase 1)
    // and its residues checked algebraically (Phase 2); neither step alone
    // proves the evaluations are bound to the commitments via a pairing.
    let (bg_polys, bg_rands_refs) = bit_gadget_set_polys_and_rands(bit_gadgets, bg_rands);
    let (w_bg, evals_bg, gamma_bg) =
        parallel_batch_open(&scheme.powers, &bg_polys, &bg_rands_refs, zeta, false, rng);
    ...
    // Group 2 push: bit-gadget commitments, in the exact order
    // `bit_gadget_set_polys_and_rands` used to build `bg_polys` above.
    commitments.push(bgcomms.all_commitments());
    witnesses.push(w_bg);
    points.push(zeta);
...
pub struct PipelineResult {
    ...
    pub opening_ok:        bool, // includes the Phase 3 batch_check of all 390 bit-gadget polys
    pub range_ok:          bool, // V_max + Slack_L + Slack_R (whichever apply)
    pub bit_gadgets_ok:    bool, // redundant plaintext duplicate of what opening_ok already proves for #1/#2/#8/#15/#16/#23
    pub v_max:             u64,
    pub n:                 usize,
    pub domain_size:       usize,
}
...
pub fn full_pipeline<R: Rng>(book: &OrderBook, rng: &mut R) -> PipelineResult {
    ...
    // Bit gadgets are built and committed *before* Fiat-Shamir (Phase 1 fix,
    // see AllBitGadgetCommitments) so their commitments are absorbed into
    // the challenge that derives zeta, same as every other committed
    // polynomial. `bg_rands` now feeds Phase 3 below (compute_all_opening_proofs),
    // which actually opens these polynomials against a pairing instead of
    // just absorbing their commitments and checking residues algebraically.
    ...
    // Phase 3 is done: `opening_ok` above now includes a real batch_check
    // (pairing-verified) opening of all 390 bit-gadget polynomials, so
    // #1/#2/#8/#15/#16/#23 are cryptographically bound like the other 26
    // constraints. This plaintext recon_ok/bool_ok check is kept as a
    // redundant prover-side sanity duplicate, the same role Layer 4 plays
    // for the other 14 constraints: cheap, checks every domain point (not
    // just zeta), but not what a real verifier runs.
...
// ===========================================================================
// External Proof — serializable, independently-verifiable proof object
// ===========================================================================
//
// Everything above (`full_pipeline`, `verify_all_openings`, `verify_all`, ...)
// runs prover and verifier in the same process on live Rust values
// (`OrderBook`, `Polynomials`, ...) -- useful for benchmarking and
// self-testing, but it never proves that a proof travels as data: nothing is
// serialized, and no verifier re-derives its own Fiat-Shamir challenges from
// only the transcript-visible commitments/evaluations. `ExternalProof` +
// `verify_external` close that gap: they operate on a `CanonicalSerialize`
// struct built from only public data (disclosed order-book scalars + KZG
// commitments + opened evaluations), independently recompute zeta and alpha,
// and re-check every residue and pairing from a deserialized copy that has
// no access to the prover's private polynomials or in-memory state.
//
// One residue is a known, disclosed exception: constraint #8's own value
// polynomial (`ceiling_value` = V_max - Min(X), zero-padded past tick n --
// see `BitGadget::value`'s doc comment) is never itself committed or opened,
// only its bit-decomposition is. There is therefore no KZG-authenticated
// evaluation of it for an external verifier to check the reconstruction
// residue against, so `verify_external` checks all 211 other bit-gadget
// residues (booleanity for all 6 gadgets + reconstruction for 5 of 6) and
// reports the gap explicitly via `ExternalVerifyReport::ceiling_gap` rather
// than skip it silently or fake a value.

/// Serializable mirror of `gadgets::utils::BatchCheckProof` (which derives
/// no serialization traits, and whose `OpenEval` enum panics on the wrong
/// variant). Only the `OpenEval::Plain` shape is used anywhere in this
/// codebase (`hiding_evals` is always passed `false`), so this format is a
/// lossless round-trip of every `BatchCheckProof<Bn254>` this crate ever
/// produces, not a general-purpose one.
#[derive(CanonicalSerialize, CanonicalDeserialize, Clone)]
pub struct SerBatchCheckProof {
    pub commitments: Vec<Vec<G1Affine>>,
    pub witnesses:   Vec<G1Affine>,
    pub points:      Vec<F>,
    pub open_evals:  Vec<Vec<(F, F)>>, // (value, blinding) -- OpenEval::Plain only
    pub gammas:      Vec<F>,
}

impl SerBatchCheckProof {
    pub fn from_proof(p: &BatchCheckProof<Bn254>) -> Self {
        SerBatchCheckProof {
            commitments: p.commitments.iter()
                .map(|group| group.iter().map(|c| c.0).collect())
                .collect(),
            witnesses: p.witnesses.iter().map(|w| w.into_affine()).collect(),
            points:    p.points.clone(),
            open_evals: p.open_evals.iter()
                .map(|group| group.iter().map(|e| e.into_plain_value()).collect())
                .collect(),
            gammas: p.gammas.clone(),
        }
    }

    pub fn to_proof(&self) -> BatchCheckProof<Bn254> {
        BatchCheckProof {
            commitments: self.commitments.iter()
                .map(|group| group.iter().map(|a| Commitment(*a)).collect())
                .collect(),
            witnesses: self.witnesses.iter().map(|a| G1Projective::from(*a)).collect(),
            points:    self.points.clone(),
            open_evals: self.open_evals.iter()
                .map(|group| group.iter().map(|(v, r)| OpenEval::Plain(*v, *r)).collect())
                .collect(),
...
/// Everything a verifier needs to know about the order book that isn't
/// itself KZG-committed -- mirrors `OrderBook`'s "Public scalars disclosed
/// in the clearing receipt" block exactly, with none of the private witness
/// columns.
#[derive(CanonicalSerialize, CanonicalDeserialize, Clone)]
...
/// A `CanonicalSerialize`-able clearing proof: public inputs plus every KZG
/// opening group the pipeline produces. `main_proof` carries Group 0 (23
/// polys @ zeta), Group 1 (3 polys @ omega*zeta), Group 2 (390 bit-gadget
/// polys @ zeta), and, if applicable, the #29/#30 cliff-value openings, in
/// that fixed order -- exactly `OpeningsResult::proof`'s group layout from
/// `compute_all_opening_proofs`.
#[derive(CanonicalSerialize, CanonicalDeserialize, Clone)]
pub struct ExternalProof {
    pub public:       PublicInputs,
    pub main_proof:    SerBatchCheckProof,
    pub v_max_range:   SerBatchCheckProof,
    pub slack_l_range: Option<SerBatchCheckProof>,
    pub slack_r_range: Option<SerBatchCheckProof>,
}

/// Package the outputs of one `full_pipeline`-shaped run into an
/// `ExternalProof`. Takes only the pieces `verify_external` actually needs
/// -- no private `Polynomials`/`OrderBook` witness columns are reachable
/// from any argument here.
pub fn assemble_external_proof(
    book:          &OrderBook,
    opr:           &OpeningsResult,
    v_max_proof:   &BatchCheckProof<Bn254>,
    slack_l_proof: Option<&BatchCheckProof<Bn254>>,
    slack_r_proof: Option<&BatchCheckProof<Bn254>>,
) -> ExternalProof {
    ExternalProof {
        public: PublicInputs {
            n:               book.n,
            domain_size:     book.domain_size,
            v_max:           book.v_max,
            v_min_delta:     book.v_min_delta,
            c:               book.c,
            d:               book.d,
            p_star:          book.p_star,
            has_left_cliff:  book.has_left_cliff,
            has_right_cliff: book.has_right_cliff,
        },
        main_proof:    SerBatchCheckProof::from_proof(&opr.proof),
        v_max_range:   SerBatchCheckProof::from_proof(v_max_proof),
        slack_l_range: slack_l_proof.map(SerBatchCheckProof::from_proof),
        slack_r_range: slack_r_proof.map(SerBatchCheckProof::from_proof),
    }
}

/// Build a dummy `OrderBook` carrying only the disclosed public scalars, for
/// feeding into `mask_p_poly`/`mask_v_poly`/`sd_poly` (which read only
/// `n`/`c`/`d`/`p_star`/`v_max`/`v_min_delta`/cliff flags -- never the
/// private witness columns, left empty here).
fn public_order_book(p: &PublicInputs) -> OrderBook {
    OrderBook {
        n: p.n, domain_size: p.domain_size, prices: Vec::new(),
        b: Vec::new(), a: Vec::new(), acc_b: Vec::new(), acc_a: Vec::new(),
        min_x: Vec::new(), surp_b: Vec::new(), surp_a: Vec::new(),
        delta: Vec::new(), chk_d: Vec::new(),
        v_max: p.v_max, v_min_delta: p.v_min_delta,
        c: p.c, d: p.d, p_star: p.p_star,
        slack_l: 0, slack_r: 0,
        has_left_cliff: p.has_left_cliff, has_right_cliff: p.has_right_cliff,
    }
}

/// Recompute one bit-gadget instance's BIT_WIDTH booleanity residues (always
/// checkable) plus its single reconstruction residue (checkable only if
/// `value_at_zeta` is `Some`, i.e. only if the gadget's own value polynomial
/// was independently KZG-opened elsewhere -- see the module doc's #8/ceiling
/// exception). `evals` is this gadget's 65-entry slice of Group 2's opened
/// evaluations, in the same (bits, recon, bools) order
/// `bit_gadget_residues_and_evals` produced them in.
fn ext_bit_gadget_residues_ok(
    evals:         &[(F, F)],
    value_at_zeta: Option<F>,
    mask_at_zeta:  Option<F>,
    zh_at_zeta:    F,
) -> bool {
    assert_eq!(evals.len(), 1 + 2 * BIT_WIDTH, "bit-gadget eval slice has the wrong width");
    let bits_at_zeta:    Vec<F> = evals[0..BIT_WIDTH].iter().map(|(v, _)| *v).collect();
    let q_recon_at_zeta: F      = evals[BIT_WIDTH].0;
    let q_bools_at_zeta: Vec<F> = evals[BIT_WIDTH + 1..].iter().map(|(v, _)| *v).collect();

    let bools_ok = bits_at_zeta.iter().zip(q_bools_at_zeta.iter()).all(|(&b_at, &q_at)| {
        let v_bool = b_at * (b_at - F::one());
        (v_bool - q_at * zh_at_zeta).is_zero()
    });

    let recon_ok = match value_at_zeta {
        None => true, // unverifiable gap -- see ExternalVerifyReport::ceiling_gap
        Some(value_at_zeta) => {
            let mut recon_at_zeta = F::zero();
            let mut weight = F::one();
            for &bv in &bits_at_zeta {
            ...
            let diff = value_at_zeta - recon_at_zeta;
            let gated = match mask_at_zeta {
                Some(m) => m * diff,
                None    => diff,
            };
            (gated - q_recon_at_zeta * zh_at_zeta).is_zero()
        }
    };
    bools_ok && recon_ok
}

/// Result of `verify_external`: every check broken out individually rather
/// than collapsed into one boolean, so a caller (or a human auditor) can see
/// exactly which part of the proof failed instead of a single opaque PASS/FAIL.
#[derive(Debug, Clone)]
pub struct ExternalVerifyReport {
    pub zeta_ok:          bool, // recomputed zeta matches every opening point derived from it
    pub alpha_derived:    F,    // recomputed alpha, reported for audit-trail purposes only --
                                 // not itself gated on, since no claimed alpha is ever sent as
                                 // proof data and every residue below is checked individually
                                 // (strictly stronger than an alpha-weighted batched check)
    pub residues_ok:      bool, // all 14 main + 211/212 bit-gadget residues == 0
    pub main_pairing_ok:  bool, // batch_check over Group 0/1/2 (+ cliff groups if present)
    pub v_max_pairing_ok: bool,
    pub v_max_bound_ok:   bool,
    pub slack_l_ok:       Option<bool>, // None iff no left cliff
    pub slack_r_ok:       Option<bool>, // None iff no right cliff
    pub ceiling_gap:      bool, // always true -- see module doc; #8's reconstruction
                                 // residue is not independently checkable with the
                                 // current commitment structure
}

impl ExternalVerifyReport {
    pub fn all_pass(&self) -> bool {
        self.zeta_ok
            && self.residues_ok
            && self.main_pairing_ok
            && self.v_max_pairing_ok
            && self.v_max_bound_ok
            && self.slack_l_ok.unwrap_or(true)
            && self.slack_r_ok.unwrap_or(true)
    }
}

/// Independently verify an `ExternalProof` against only `vk` (the shared,
/// out-of-band KZG verifier key -- analogous to `zeeperio`'s
/// `Srs::load_from_dir`, legitimately public setup data, not part of any one
/// proof). Never touches an `OrderBook`, `Polynomials`, or any other
/// prover-side value; the caller is expected to have deserialized `proof`
/// from bytes, so this function proves the proof is verifiable as data, not
/// just self-consistent within one process's memory.
///
/// Returns `Err` only for a structurally malformed proof (wrong number of
/// opening groups/evaluations for the declared public inputs); a
/// well-formed but cryptographically invalid proof returns `Ok` with
/// `ExternalVerifyReport::all_pass() == false`.
pub fn verify_external(
    proof: &ExternalProof,
    vk:    &VerifierKey<Bn254>,
) -> Result<ExternalVerifyReport, String> {
    let pub_ = &proof.public;
    if pub_.n == 0 || pub_.n > pub_.domain_size {
        return Err(format!("invalid public inputs: n={} domain_size={}", pub_.n, pub_.domain_size));
    }
    let book   = public_order_book(pub_);
    let domain = build_domain(pub_.domain_size);
    let elems: Vec<F> = domain.elements().collect();

    let mp = &proof.main_proof;
    let expected_extra_groups =
        pub_.has_left_cliff as usize + pub_.has_right_cliff as usize;
    if mp.commitments.len() != 3 + expected_extra_groups
        || mp.commitments[0].len() != 23
        || mp.commitments[1].len() != 3
        || mp.commitments[2].len() != 390
    {
        return Err(format!(
            "main_proof does not have the expected 23/3/390(+{}) group shape",
            expected_extra_groups
        ));
    }
    if mp.open_evals.len() != mp.commitments.len()
        || mp.open_evals[0].len() != 23
        || mp.open_evals[1].len() != 3
        || mp.open_evals[2].len() != 390
    {
        return Err("main_proof open_evals do not match its commitment group shape".to_string());
    }

    // --- Step 1: re-derive zeta from only the committed data (Group 0's
    // 23 commitments, then Group 2's 390 bit-gadget commitments -- the exact
    // order `fiat_shamir_prove` absorbed them in) ---
    let mut transcript = Transcript::new();
    let mut absorbed: Vec<G1Affine> = mp.commitments[0].clone();
    absorbed.extend(mp.commitments[2].iter().cloned());
    transcript.append_affines::<Bn254>(&absorbed);
    let zeta = transcript.append_and_digest::<Bn254>("zeta".to_string());

    let omega       = domain.group_gen();
    let omega_zeta  = omega * zeta;
    let domain_size = pub_.domain_size;

    let mut zeta_ok = mp.points[0] == zeta && mp.points[1] == omega_zeta && mp.points[2] == zeta;
    let mut idx = 3;
    if pub_.has_left_cliff {
        if pub_.c == 0 { return Err("has_left_cliff but c == 0".to_string()); }
        zeta_ok &= mp.points.get(idx) == Some(&elems[pub_.c - 1]);
        idx += 1;
    }
    if pub_.has_right_cliff {
        if pub_.d + 1 >= domain_size { return Err("has_right_cliff but d+1 out of range".to_string()); }
        zeta_ok &= mp.points.get(idx) == Some(&elems[pub_.d + 1]);
        idx += 1;
    }

    // --- Step 2: pull the 30 main + 390 bit-gadget evaluations straight out
    // of the (deserialized) opening proof -- these are exactly the values
    // `batch_check` (Step 5 below) pairing-verifies against `mp`'s
    // commitments, so there is nothing left to trust the prover about here.
    let g0 = &mp.open_evals[0];
    let ev = |i: usize| g0[i].0;
    let (b_at_zeta, a_at_zeta, acc_b_at_zeta, acc_a_at_zeta, min_x_at_zeta) =
        (ev(0), ev(1), ev(2), ev(3), ev(4));
    let (surp_b_at_zeta, surp_a_at_zeta, delta_at_zeta, chk_d_at_zeta) =
        (ev(5), ev(6), ev(7), ev(8));
    let q_acc_a_init_at_zeta    = ev(9);
    let q_acc_a_rec_at_zeta     = ev(10);
    let q_acc_b_init_at_zeta    = ev(11);
    let q_acc_b_rec_at_zeta     = ev(12);
    let q_kl_at_zeta            = ev(13);
    let q_delta_def_at_zeta     = ev(14);
    let q_chkd_bool_at_zeta     = ev(15);
    let q_chkd_correct_at_zeta  = ev(16);
    let q_chkd_contain_at_zeta  = ev(17);
    let q_plateau_left_at_zeta  = ev(18);
    let q_plateau_right_at_zeta = ev(19);
    let q_valley_pin_at_zeta    = ev(20);
    let q_mask_v_contain_at_zeta = ev(21);
    let q_sd_def_at_zeta         = ev(22);

    let g1 = &mp.open_evals[1];
    let (acc_a_at_omega_zeta, a_at_omega_zeta, acc_b_at_omega_zeta) = (g1[0].0, g1[1].0, g1[2].0);

    let g2 = &mp.open_evals[2];

    // --- Step 3: public polys the verifier recomputes itself (never committed) ---
    let mask_p_at_zeta = mask_p_poly(&book, &domain).evaluate(&zeta);
    let mask_v_at_zeta = mask_v_poly(&book, &domain).evaluate(&zeta);
    let sd_at_zeta      = sd_poly(&book, &domain).evaluate(&zeta);

    let zh_at_zeta      = zeta.pow([domain_size as u64]) - F::one();
    let omega_0         = elems[0];
    let omega_last_data = elems[pub_.n - 1];
    let omega_last_dom  = elems[domain_size - 1];
    let v_max       = F::from(pub_.v_max);
    let v_min_delta = F::from(pub_.v_min_delta);

    // --- Step 4: recompute all 14 main residues (identical formulas to
    // `fiat_shamir_prove`'s Step 3), checked individually rather than folded
    // by alpha -- strictly stronger than the prover's own batch_ok check ---
    let r1 = (acc_a_at_zeta - a_at_zeta) - q_acc_a_init_at_zeta * (zeta - omega_0);
    let v2 = (zeta - omega_last_data) * (acc_a_at_omega_zeta - acc_a_at_zeta - a_at_omega_zeta);
    let r2 = v2 - q_acc_a_rec_at_zeta * zh_at_zeta;
    let r3 = (acc_b_at_zeta - b_at_zeta) - q_acc_b_init_at_zeta * (zeta - omega_last_data);
    ...
    let r9 = v9 - q_chkd_contain_at_zeta * zh_at_zeta;
    let v10 = min_x_at_zeta - v_max;
    let r10 = v10 - q_plateau_left_at_zeta * (zeta - elems[pub_.c]);
    let r11 = v10 - q_plateau_right_at_zeta * (zeta - elems[pub_.d]);
    let v12 = delta_at_zeta - v_min_delta;
    let r12 = v12 - q_valley_pin_at_zeta * (zeta - elems[pub_.p_star]);
    let v13 = mask_v_at_zeta * (F::one() - chk_d_at_zeta);
    let r13 = v13 - q_mask_v_contain_at_zeta * zh_at_zeta;
    let v14 = sd_at_zeta - mask_v_at_zeta * delta_at_zeta;
    let r14 = v14 - q_sd_def_at_zeta * zh_at_zeta;
    let main_residues_ok =
        [r1, r2, r3, r4, r5, r6, r7, r8, r9, r10, r11, r12, r13, r14].iter().all(|r| r.is_zero());

    // --- Step 5: recompute the 211/212 bit-gadget residues. Per-gadget
    // slice order matches `bit_gadget_set_polys_and_rands`/`AllBitGadgetCommitments`
    // exactly: bid_range, ask_range, ceiling, surp_b_nn, surp_a_nn, delta_floor.
    let per_gadget = 1 + 2 * BIT_WIDTH;
    let slice = |k: usize| &g2[k * per_gadget..(k + 1) * per_gadget];
    let bid_ok  = ext_bit_gadget_residues_ok(slice(0), Some(b_at_zeta), None, zh_at_zeta);
    let ask_ok  = ext_bit_gadget_residues_ok(slice(1), Some(a_at_zeta), None, zh_at_zeta);
    let ceil_ok = ext_bit_gadget_residues_ok(slice(2), None, None, zh_at_zeta); // #8 gap
    let sb_ok   = ext_bit_gadget_residues_ok(slice(3), Some(surp_b_at_zeta), None, zh_at_zeta);
    let sa_ok   = ext_bit_gadget_residues_ok(slice(4), Some(surp_a_at_zeta), None, zh_at_zeta);
    let df_ok   = ext_bit_gadget_residues_ok(
        slice(5), Some(delta_at_zeta - v_min_delta), Some(mask_p_at_zeta), zh_at_zeta,
    );
    let residues_ok = main_residues_ok && bid_ok && ask_ok && ceil_ok && sb_ok && sa_ok && df_ok;

    // --- Step 6: re-derive alpha (audit trail only, see field doc) ---
    let mut fs_transcript = Transcript::new();
    for val in &[
        zeta,
        b_at_zeta, a_at_zeta, acc_b_at_zeta, acc_a_at_zeta, min_x_at_zeta,
        surp_b_at_zeta, surp_a_at_zeta, delta_at_zeta, chk_d_at_zeta, mask_p_at_zeta,
        ...
        fs_transcript.append_message(format!("{:?}", val));
    }
    for (v, _blind) in g2 {
        fs_transcript.append_message(format!("{:?}", v));
    }
    let alpha_derived = fs_transcript.append_and_digest::<Bn254>("alpha".to_string());

    // --- Step 7: pairing checks, on the deserialized proof only ---
    let main_pairing_ok = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        let mut vrng = ark_std::test_rng();
        batch_check(vk, &mp.to_proof(), &mut vrng);
    }))
    .is_ok();

    let v_max_bcp = proof.v_max_range.to_proof();
    let v_max_pairing_ok = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        let mut vrng = ark_std::test_rng();
        range_gadget::verify::<Bn254, _>(vk.clone(), &v_max_bcp, range_domain(), &mut vrng); // GADGET: range
    }))
    .is_ok();
    let v_max_bound_ok = v_max_pairing_ok && {
        let (t_at_one, _) = v_max_bcp.open_evals[2][0].into_plain_value();
        t_at_one == v_max
    };

    // Cliff values are opened (pairing-verified as part of `main_pairing_ok`
    // above) at ω^{c-1}/ω^{d+1}; Slack_{L,R} are not independently committed
    // (see `OrderBook`'s doc comment), so the expected slack a range proof
    // must bind to is derived here from the disclosed V_max and that opened
    // value, not trusted from any separately-disclosed slack scalar.
    let mut extra_idx = 3;
    let min_at_c_minus_1 = if pub_.has_left_cliff {
        let v = mp.open_evals[extra_idx][0].0;
        extra_idx += 1;
        Some(v)
    } else { None };
    let min_at_d_plus_1 = if pub_.has_right_cliff {
        let v = mp.open_evals[extra_idx][0].0;
        Some(v)
    } else { None };

    let slack_l_ok = proof.slack_l_range.as_ref().map(|sl| {
        let bcp = sl.to_proof();
        let pairing_ok = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            let mut vrng = ark_std::test_rng();
            range_gadget::verify::<Bn254, _>(vk.clone(), &bcp, range_domain(), &mut vrng); // GADGET: range
        }))
        .is_ok();
        let expected = pub_.v_max - 1 - to_u64(min_at_c_minus_1.unwrap_or(F::zero()));
        let (t_at_one, _) = bcp.open_evals[2][0].into_plain_value();
        pairing_ok && min_at_c_minus_1.is_some() && t_at_one == F::from(expected)
    });
    let slack_r_ok = proof.slack_r_range.as_ref().map(|sr| {
        let bcp = sr.to_proof();
        let pairing_ok = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            let mut vrng = ark_std::test_rng();
            range_gadget::verify::<Bn254, _>(vk.clone(), &bcp, range_domain(), &mut vrng); // GADGET: range
        ...
        .is_ok();
        let expected = pub_.v_max - 1 - to_u64(min_at_d_plus_1.unwrap_or(F::zero()));
        let (t_at_one, _) = bcp.open_evals[2][0].into_plain_value();
        pairing_ok && min_at_d_plus_1.is_some() && t_at_one == F::from(expected)
    });

    Ok(ExternalVerifyReport {
        zeta_ok,
        alpha_derived,
        residues_ok,
        main_pairing_ok,
...
mod tests {
    ...
    /// Phase 3 negative control: swap one bit-gadget commitment for an
    /// unrelated but otherwise-valid one right before opening. If Phase 3's
    /// `batch_check` on Group 2 (the 390 bit-gadget polynomials) is actually
    /// load-bearing -- i.e. it really pairing-verifies that the zeta
    /// evaluations came from the *committed* polynomials, not just
    /// self-consistent prover-chosen values -- this must make `opening_ok`
    /// false. If it stayed true, Group 2 would be decorative.
    #[test]
    fn bit_gadget_opening_rejects_swapped_commitment() {
        ...
        let scheme      = KzgScheme::new(domain_size - 1, &mut rng);

        let (wcomms, w_rands) = commit_all(&scheme, &polys, &mut rng);
        let (quots, _qcheck)  = compute_quotients(&book, &polys, &mask_p, &domain, n);
        let (qcomms, q_rands) = commit_quotients(&scheme, &quots, &mut rng);

        let bit_gadgets = build_bit_gadget_set(&book, &polys, &mask_p, &domain);
        let (mut bg_comms, bg_rands) = commit_all_bit_gadgets(&scheme, &bit_gadgets, &mut rng);

        let fs = fiat_shamir_prove(
            &book, &polys, &mask_p, &quots, &wcomms, &qcomms,
            &bit_gadgets, &bg_comms, &domain, n,
        );

        // Tamper: swap the first bit commitment of bid_range for an
        // unrelated, otherwise-legitimate commitment (C_B). The opened
        // evaluation still comes from the real (un-tampered) polynomial via
        // `bg_rands`/`bit_gadgets`, so this isolates whether `batch_check`
        // actually checks commitment-to-evaluation binding for this group.
        let honest_commitment = bg_comms.bid_range.c_bits[0].clone();
        bg_comms.bid_range.c_bits[0] = wcomms.c_b.clone();
        assert_ne!(
            honest_commitment, bg_comms.bid_range.c_bits[0],
        ...
        let opr = compute_all_opening_proofs(
            &scheme, &book, &polys, &quots, &wcomms, &qcomms,
            &w_rands, &q_rands, &fs, &domain,
            &bit_gadgets, &bg_comms, &bg_rands, &mut rng,
        );
        let opening_ok = verify_all_openings(&scheme, &opr, &mut rng);
        assert!(
            !opening_ok,
            "batch_check accepted a tampered bit-gadget commitment -- Phase 3 opening is not load-bearing"
        );
    }

    /// Chunk 1 happy path: an honestly-produced proof, serialized to bytes,
    /// deserialized back with no access to any prover-side value, still
    /// verifies via `verify_external` alone.
    #[test]
    fn external_proof_round_trips_and_verifies_honest_pipeline() {
        let book        = OrderBook::hardcoded_21tick();
        let n           = book.n;
        let domain_size = book.domain_size;
        let domain      = build_domain(domain_size);
        ...
        let (qcomms, q_rands) = commit_quotients(&scheme, &quots, &mut rng);
        let bit_gadgets = build_bit_gadget_set(&book, &polys, &mask_p, &domain);
        let (bg_comms, bg_rands) = commit_all_bit_gadgets(&scheme, &bit_gadgets, &mut rng);
        ...
        let v_max_proof = prove_range16(&scheme, book.v_max, &mut rng);
        let (slack_l_proof, slack_r_proof) =
            prove_cliff_slack(&scheme, &book, opr.min_at_c_minus_1, opr.min_at_d_plus_1, &mut rng);

        let ext = assemble_external_proof(
            &book, &opr, &v_max_proof, slack_l_proof.as_ref(), slack_r_proof.as_ref(),
        );

        let mut wire = Vec::new();
        ext.serialize_compressed(&mut wire).expect("serialize ExternalProof");
        let round_tripped =
            ExternalProof::deserialize_compressed(&wire[..]).expect("deserialize ExternalProof");

        let report = verify_external(&round_tripped, &scheme.vk).expect("well-formed proof shape");
        assert!(report.zeta_ok, "zeta re-derivation should match an honest proof");
        assert!(report.residues_ok, "all checkable residues should be zero for an honest proof");
        assert!(report.main_pairing_ok);
        assert!(report.v_max_pairing_ok && report.v_max_bound_ok);
        assert!(report.all_pass(), "verify_external should accept an honest, round-tripped proof");
    }

    /// The gap Chunk 1 closes: nothing in `verify_all_openings`/`batch_check`
    /// checks that the opening point actually equals the honest Fiat-Shamir
    /// zeta -- it only checks that (point, commitment, evaluation, witness)
    /// are mutually consistent for *whatever* point the prover opened at.
    /// Since every witness polynomial here genuinely satisfies its identity
    /// everywhere (not just at one point), opening at a prover-chosen point
    /// instead of the transcript-derived one still produces a proof that
    /// `verify_all_openings` happily accepts. `verify_external` must catch
    /// this by re-deriving zeta itself and checking it against the proof's
    /// own opening points, regardless of whether the pairing checks pass.
    #[test]
    fn external_verify_rejects_non_fiat_shamir_opening_point() {
        let book        = OrderBook::hardcoded_21tick();
        let n           = book.n;
        let domain_size = book.domain_size;
        let domain      = build_domain(domain_size);
        ...
        let mut fs = fiat_shamir_prove(
            &book, &polys, &mask_p, &quots, &wcomms, &qcomms,
            &bit_gadgets, &bg_comms, &domain, n,
        );
        let honest_zeta = fs.zeta;
        // Forge zeta to an arbitrary, prover-chosen point instead of the
        // transcript-derived one. Every polynomial here is honestly correct
        // everywhere, so this alone doesn't break batch_check's pairing math.
        fs.zeta = honest_zeta + F::from(12345u64);
        assert_ne!(fs.zeta, honest_zeta);
        ...
        assert!(
            verify_all_openings(&scheme, &opr, &mut rng),
            "test fixture bug: a forged-zeta opening should still pairing-verify -- that's the gap"
        );

        let v_max_proof = prove_range16(&scheme, book.v_max, &mut rng);
        let ext = assemble_external_proof(&book, &opr, &v_max_proof, None, None);
        let report = verify_external(&ext, &scheme.vk).expect("well-formed proof shape");
        assert!(!report.zeta_ok, "verify_external accepted a non-Fiat-Shamir opening point");
        assert!(!report.all_pass(), "verify_external accepted a proof opened at a forged point");
    }
}
