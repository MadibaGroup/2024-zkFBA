...
use zk_fba_csv_full::{
    ...
    assemble_external_proof, verify_external, ExternalProof,
    KzgScheme, OrderBook,
};
    ...
    -> (f64, u64, bool)
{
    ...
    // -- Layer 5: External Auditability ----------------------------------------
    // Package a serializable, self-contained proof object and hand it to an
    // independent verifier that only sees public data (VK + wire bytes) and
    // re-derives the Fiat-Shamir transcript itself, rather than trusting the
    // same-process opr/fs values used above. This is the gap the zeeperio-style
    // external-verifier comparison flagged: verify_all_openings above never
    // checks that the opening point actually equals the honest Fiat-Shamir
    // zeta -- an independent verifier must re-derive it from scratch.
    println!("\n--- LAYER 5  External Auditability (independent verifier) ---");
    let ext_proof = assemble_external_proof(
        book, &opr, &range_proof, slack_l_proof.as_ref(), slack_r_proof.as_ref(),
    );
    let mut wire = Vec::new();
    ext_proof.serialize_compressed(&mut wire).expect("serialize ExternalProof");
    let round_tripped = ExternalProof::deserialize_compressed(&wire[..])
        .expect("deserialize ExternalProof");
    let ext_report = verify_external(&round_tripped, &scheme.vk)
        .expect("verify_external structural check");
    println!("  wire-format proof size (ark-serialize, compressed): {} bytes  (vs {} bytes element-counted above; difference is serialization overhead: length prefixes, uncompressed Fr option flags, PublicInputs)",
        wire.len(), elementcount_bytes);
    println!("  zeta re-derived from public commitments matches prover's:  {}",
        if ext_report.zeta_ok { "PASS" } else { "FAIL !!!" });
    println!("  all 14 + 6x(bit-gadget) algebraic residues re-checked:      {}",
        if ext_report.residues_ok { "PASS" } else { "FAIL !!!" });
    println!("  main batch_check (independent pairing check):              {}",
        if ext_report.main_pairing_ok { "PASS" } else { "FAIL !!!" });
    println!("  V_max range proof (pairing + bound to disclosed V_max):    {}",
        if ext_report.v_max_pairing_ok && ext_report.v_max_bound_ok { "PASS" } else { "FAIL !!!" });
    println!("  slack_l range proof (bound to KZG-opened cliff Min(X)):    {}",
        match ext_report.slack_l_ok { Some(true) => "PASS", Some(false) => "FAIL !!!", None => "N/A" });
    println!("  slack_r range proof (bound to KZG-opened cliff Min(X)):    {}",
        match ext_report.slack_r_ok { Some(true) => "PASS", Some(false) => "FAIL !!!", None => "N/A" });
    println!("  KNOWN GAP -- #8 (ceiling, V_max - Min(X)): value polynomial is never");
    println!("    itself committed/opened via KZG, so its reconstruction residue cannot");
    println!("    be independently re-checked here (booleanity of its 32 bits still is).");
    let external_ok = ext_report.all_pass();
    println!("  EXTERNAL VERIFY ALL PASS: {}", if external_ok { "YES \u{2713}" } else { "NO \u{2717}" });

    // -- Layer 6: Aggregated Bit-Gadget Pipeline (proof-size reduction) ---------
    // A completely separate, self-contained pipeline (zk_fba_csv_full::agg_pipeline,
    // ported verbatim -- lib.rs above is untouched for this) that replaces the
    // 390 individually-committed/opened bit-decomposition polynomials (6
    // instances x 65: 32 bits + 1 recon quotient + 32 individual booleanity
    // quotients) with an RLC-aggregated 204 (6 x 34: 32 bits + 1 recon quotient
    // + 1 aggregated booleanity quotient). Re-runs interpolation/commit/open
    // from scratch on the same book -- this is not a replacement for the run
    // above, it's an independent alternative construction, reported separately.
    println!("\n--- LAYER 6  Aggregated Bit-Gadget Pipeline (Chunk 4: proof size) ---");
    let t = Instant::now();
    let agg_result = zk_fba_csv_full::agg_pipeline::agg_full_pipeline(book, rng);
    let l6_total = t.elapsed();
    let agg_bytes = agg_result.proof_bytes();
    println!("  bit-gadget polys: {} (vs 390 original)   total committed polys: {} (vs 447)",
        agg_result.num_bit_gadget_polys, agg_result.num_total_committed_polys);
    println!("  proof size: {} bytes (vs {} bytes original element-counted, vs 14,656 bytes Noir)",
        agg_bytes, elementcount_bytes);
    println!("  reduction vs original: {:.2}x smaller   still vs Noir: {:.2}x larger",
        elementcount_bytes as f64 / agg_bytes as f64,
        agg_bytes as f64 / 14656.0);
    println!("  ALL PASS: {}   total wall time: {:.3?}",
        if agg_result.all_pass() { "YES \u{2713}" } else { "NO \u{2717}" }, l6_total);
...
fn main() {
    ...
    // ── Run 2: 100-tick CSV ─────────────────────────────────────────────────
    // REAL-DATA SWAP: only this path string changed, from the synthetic
    // log-normal CSV to a real-market CSV built by a separate script
    // (../data/fetch_real_data.py + ../data/build_real_csv.py) in the exact
    // same 20-column format OrderBook::from_csv already expects -- see
    // ../data/NOTES.md. Everything downstream of this line (OrderBook::from_csv,
    // run_book, and all of lib.rs) is byte-for-byte the original code.
    let csv100 = concat!(env!("CARGO_MANIFEST_DIR"), "/../data/order_book_real_quotes_100.csv");
    let book100 = OrderBook::from_csv(csv100);
    let (t100, v_max100, ok100) = run_book("100-tick real AAPL quotes CSV", &book100, &mut rng);

    // ── Run 3: 1000-tick CSV ────────────────────────────────────────────────
    // REAL-DATA SWAP: same as Run 2, path only. NOTE: no real N=1000 dataset
    // was fetched (the ask was two N=100 real datasets -- quotes and trades),
    // so this slot now carries the second real dataset (trades) instead of a
    // 1000-tick CSV. It is still N=100, domain_size=128, same as Run 2 --
    // the comparison-table row and scaling section below use book1000.n /
    // book1000.domain_size dynamically rather than the original's hardcoded
    // 1000/1024 literals, since those specific two numbers would otherwise
    // be printed false. No other line in this file changed for that reason.
    let csv1000 = concat!(env!("CARGO_MANIFEST_DIR"), "/../data/order_book_real_trades_100.csv");
    let book1000 = OrderBook::from_csv(csv1000);
    let (t1000, v_max1000, ok1000) = run_book("100-tick real AAPL trades CSV", &book1000, &mut rng);
    ...
    println!(
        "  {:<32}  {:>6}  {:>10}  {:>9}  {:>8.1}  {}",
        "100-tick real AAPL quotes CSV", book100.n, book100.domain_size, v_max100, t100,
        if ok100 { "PASS" } else { "FAIL" }
    );
    println!(
        "  {:<32}  {:>6}  {:>10}  {:>9}  {:>8.1}  {}",
        "100-tick real AAPL trades CSV", book1000.n, book1000.domain_size, v_max1000, t1000,
    ...
    100-tick (quotes) : {:.1}x slower   (domain {}x larger: {} vs 32)
    100-tick (trades) : {:.1}x slower   (domain {}x larger: {} vs 32)",
            t100 / t21,   book100.domain_size / 32,  book100.domain_size,
            t1000 / t21, book1000.domain_size / 32, book1000.domain_size
        );
    }
    ...
    println!("  - Runs 2/3 use real AAPL market data (Databento XNAS.ITCH, 2026-06-01");
    println!("    09:30-09:35 ET) in place of the synthetic log-normal CSVs -- see");
    println!("    ../data/NOTES.md. No other logic in this file or lib.rs changed.");
    println!("{}", "=".repeat(70));
}
