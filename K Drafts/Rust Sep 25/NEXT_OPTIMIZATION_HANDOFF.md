...
**Note (this copy only, `zk_fba_real_data/rust/`):** this is an unmodified,
byte-identical copy of `~/zk_fba_csv_full/NEXT_OPTIMIZATION_HANDOFF.md` as
of 2026-08-24 -- it describes that sibling project's own optimization
history (synthetic log-normal data only, no real market data, predates
Chunks 1/4) and its "point Claude at this file" instruction below is scoped
to `zk_fba_csv_full`, not this project. For `zk_fba_real_data`'s own current
state -- real-data benchmarks, the external-verifier addition (Chunk 1), and
the proof-size-reduction pipeline (Chunk 4) -- read `../RESULTS.md` and
`../SYNTHETIC_VS_REAL_COMPARISON.md` instead. This file is kept only as
historical background for where the fixed-cost-layer diagnosis (item 3/4
