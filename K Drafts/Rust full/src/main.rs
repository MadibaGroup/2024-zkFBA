...
use zk_fba_csv_full::{
    ...
    build_bit_gadget_set, commit_bit_gadget,
    KzgScheme, OrderBook,
};
    ...
    -> (f64, u64, bool)
{
    ...
    let t = Instant::now();
    let (_bc_bid, ..)         = commit_bit_gadget(&scheme, &bit_gadgets.bid_range, rng);
    let (_bc_ask, ..)         = commit_bit_gadget(&scheme, &bit_gadgets.ask_range, rng);
    let (_bc_ceiling, ..)     = commit_bit_gadget(&scheme, &bit_gadgets.ceiling, rng);
    let (_bc_surp_b, ..)      = commit_bit_gadget(&scheme, &bit_gadgets.surp_b_nn, rng);
    let (_bc_surp_a, ..)      = commit_bit_gadget(&scheme, &bit_gadgets.surp_a_nn, rng);
    let (_bc_delta_floor, ..) = commit_bit_gadget(&scheme, &bit_gadgets.delta_floor, rng);
