...
pub fn compute_quotients(
    ...
    let (q8, r8) = poly_div_rem(&v8, &zh);

    // #20  ChkD * (1 - Mask_P) = 0
    let v9 = &polys.chk_d * &(&one_poly - mask_p);
    let (q9, r9) = poly_div_rem(&v9, &zh);
    ...
    let check = QuotientCheck {
        ...
        r_delta_def:     r6.coeffs.iter().all(|x| x.is_zero()),
        r_chkd_bool:     r7.coeffs.iter().all(|x| x.is_zero()),
        r_chkd_correct:  r8.coeffs.iter().all(|x| x.is_zero()),
        r_chkd_contain:  r9.coeffs.iter().all(|x| x.is_zero()),
        r_plateau_left:  r10.is_zero(),
        r_plateau_right: r11.is_zero(),
        r_valley_pin:    r12.is_zero(),
        r_mask_v_contain: r13.coeffs.iter().all(|x| x.is_zero()),
        r_sd_def:         r14.coeffs.iter().all(|x| x.is_zero()),
    };
    (quotients, check)
}
...
pub fn verify_all(
    ...
    ConstraintResult {
        acc_a_init: polys.acc_a.evaluate(&w0) == polys.a.evaluate(&w0),
        acc_a_recurrence: (0..n - 1).all(|i| {
            polys.acc_a.evaluate(&elems[i + 1])
                == polys.acc_a.evaluate(&elems[i]) + polys.a.evaluate(&elems[i + 1])
        }),
        acc_b_init: polys.acc_b.evaluate(&wlast) == polys.b.evaluate(&wlast),
        acc_b_recurrence: (0..n - 1).all(|i| {
            polys.acc_b.evaluate(&elems[i])
                == polys.acc_b.evaluate(&elems[i + 1]) + polys.b.evaluate(&elems[i])
        }),
        min_exclusivity: (0..n).all(|i| {
            let da = polys.acc_a.evaluate(&elems[i]) - polys.min_x.evaluate(&elems[i]);
            let db = polys.acc_b.evaluate(&elems[i]) - polys.min_x.evaluate(&elems[i]);
            (da * db).is_zero()
        }),
        delta_definition: (0..n).all(|i| {
            polys.delta.evaluate(&elems[i])
                == polys.surp_a.evaluate(&elems[i]) + polys.surp_b.evaluate(&elems[i])
        }),
        chkd_booleanity: (0..n).all(|i| {
            let v = polys.chk_d.evaluate(&elems[i]);
            (v * (v - F::one())).is_zero()
        }),
        chkd_correctness: (0..n).all(|i| {
            ((polys.delta.evaluate(&elems[i]) - v_min_delta) * polys.chk_d.evaluate(&elems[i])).is_zero()
        }),
        chkd_containment: (0..n).all(|i| {
            (polys.chk_d.evaluate(&elems[i]) * (F::one() - mask_p.evaluate(&elems[i]))).is_zero()
        }),
        plateau_left:  polys.min_x.evaluate(&elems[book.c]) == v_max,
        plateau_right: polys.min_x.evaluate(&elems[book.d]) == v_max,
        valley_pin:    polys.delta.evaluate(&elems[book.p_star]) == v_min_delta,
        inmcv_inside: (0..n).all(|i| {
            let iv = inmcv.evaluate(&elems[i]);
            let mp = mask_p.evaluate(&elems[i]);
            ((iv - v_max) * mp).is_zero()
        }),
        inmcv_outside: (0..n).all(|i| {
            let iv = inmcv.evaluate(&elems[i]);
            let mp = mask_p.evaluate(&elems[i]);
            (iv * (F::one() - mp)).is_zero()
        }),
        mask_v_booleanity: (0..n).all(|i| {
            let v = mask_v.evaluate(&elems[i]);
            (v * (v - F::one())).is_zero()
        }),
        mask_v_containment: (0..n).all(|i| {
            (mask_v.evaluate(&elems[i]) * (F::one() - polys.chk_d.evaluate(&elems[i]))).is_zero()
        }),
        sd_definition: (0..n).all(|i| {
            (sd.evaluate(&elems[i]) - mask_v.evaluate(&elems[i]) * polys.delta.evaluate(&elems[i])).is_zero()
        }),
        sd_membership: (0..n).all(|i| {
            let s = sd.evaluate(&elems[i]);
            (s * (s - v_min_delta)).is_zero()
        }),
        mask_c_booleanity: (0..n).all(|i| {
            let v = mask_c.evaluate(&elems[i]);
            (v * (v - F::one())).is_zero()
        }),
        mask_c_containment: (0..n).all(|i| {
            (mask_c.evaluate(&elems[i]) * mask_p.evaluate(&elems[i])).is_zero()
        }),
        mask_p_booleanity: (0..n).all(|i| {
            let v = mask_p.evaluate(&elems[i]);
            (v * (v - F::one())).is_zero()
        }),
    }
}
...
pub fn build_bit_gadget(
    ...
    let (q_recon, r_recon) = poly_div_rem(&gated, zh);
    let recon_ok = r_recon.coeffs.iter().all(|x| x.is_zero());
    ...
    for b in &cols.bits {
        let v = b * &(b - &one_poly);
        let (q, r) = poly_div_rem(&v, zh);
        bool_ok &= r.coeffs.iter().all(|x| x.is_zero());
        q_bools.push(q);
    }

    BitGadget { cols, q_recon, q_bools, recon_ok, bool_ok }
}
...
pub fn build_bit_gadget_set(
    ...
    let zh = build_zh_poly(domain.size());
    ...
    let bid_range = build_bit_gadget(&bid_bits, &polys.b, None, &zh, domain);

    let ask_bits: Vec<u64> = book.a.iter().map(|&f| to_u64(f)).collect();
    assert_fits_bit_width(&ask_bits, "#2 Ask range");
    let ask_range = build_bit_gadget(&ask_bits, &polys.a, None, &zh, domain);
    ...
    let ceiling = build_bit_gadget(&ceiling_bits, &ceiling_value, None, &zh, domain);
    ...
    let surp_b_nn = build_bit_gadget(&surp_b_bits, &polys.surp_b, None, &zh, domain);
    ...
    let surp_a_nn = build_bit_gadget(&surp_a_bits, &polys.surp_a, None, &zh, domain);
    ...
    let delta_floor =
        build_bit_gadget(&delta_floor_bits, &delta_floor_value, Some(mask_p), &zh, domain);
...
pub fn fiat_shamir_prove(
    ...
    // Step 1: absorb all 23 commitments → zeta
    let mut transcript = Transcript::new();
    transcript.append_affines::<Bn254>(&vec![
    ...
    let r = vec![r1, r2, r3, r4, r5, r6, r7, r8, r9, r10, r11, r12, r13, r14];
...
pub fn full_pipeline<R: Rng>(book: &OrderBook, rng: &mut R) -> PipelineResult {
    ...
    let fs          = fiat_shamir_prove(book, &polys, &mask_p, &quots, &wcomms, &qcomms, &domain, n);
    ...
    let _ = commit_bit_gadget(&scheme, &bit_gadgets.bid_range, rng);
    let _ = commit_bit_gadget(&scheme, &bit_gadgets.ask_range, rng);
    let _ = commit_bit_gadget(&scheme, &bit_gadgets.ceiling, rng);
    let _ = commit_bit_gadget(&scheme, &bit_gadgets.surp_b_nn, rng);
    let _ = commit_bit_gadget(&scheme, &bit_gadgets.surp_a_nn, rng);
    let _ = commit_bit_gadget(&scheme, &bit_gadgets.delta_floor, rng);
