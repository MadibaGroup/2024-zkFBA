...
**So: adding explicit non-negativity to both sides**, reusing the same bit-decomposition gadget already used for `Vceiling` (3):
...
($Mask_{p^*}(X) = Z_H(X)/(X-\omega^{p^*})$, the single-point mask, pinning the minimum to the disclosed clearing price specifically, so the `Vsurp` equation, which as written defines a slack array over the whole plateau rather than pinning a single winning point.)
...
plus the transition equation. PLONKbook's own Plookup specification requires **both** boundary conditions on the grand product:
...
Treating masks ($Mask_{[c,d]}$, $Mask_i = Z_H(X)/(X-\omega^i)$) as **public, verifier-computable helpers** built directly from disclosed indices ($c,d,c-1,d+1,p^*$), no commitment, no extra proof of well-formedness needed, since the verifier can just compute them. Might require additional well-formedness constraints, proving each mask is genuinely a 0/1 indicator over the right interval.

**so:** keep masks public: it's strictly cheaper (no commitment, no extra constraint) and the values $c,d$ are opened in plaintext elsewhere in the protocol anyway, so nothing is lost.
